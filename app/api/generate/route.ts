...
export async function POST(req: Request) {
  ...
  try {
    if (m === "llama") {
      ...
      // Now using GPT-4o Vision instead of Ollama
      if (!image) {
        ...
        try {
          ...
          if (!supabaseUrl || !supabaseKey) {
            output = `[BLIP Error] Database not configured. Please check your Supabase setup.`;
          } else {
            ...
            if (!apiKey) {
              output = `[BLIP Error] OpenAI API key not found. Please add your OpenAI API key in the AI Settings page or contact admin to set system-wide key.`;
            } else {
              ...
              const analysis = openaiData.choices?.[0]?.message?.content || "Image analysis completed but no response received.";
              output = analysis;
            }
          }
        } catch (error: any) {
      ...
      if (!image) {
        ...
        try {
          ...
          // Use OLLAMA_URL environment variable instead of hardcoded localhost
          ...
          if (error.message && (error.message.includes('fetch failed') || error.message.includes('ECONNREFUSED') || error.message.includes('not running'))) {
            output = `[LLaVA Error] Ollama is not running or not accessible at ${OLLAMA_URL}. Please make sure Ollama is installed and running, then try again.`;
          } else {
